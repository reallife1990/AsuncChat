...
def work(params):
    ...
    if len(params[0]) < 1:
        ...
        ANS = params[0]
    logs.server_logger.debug(f'слушаем : {ANS}:{params[1]}')
    ...
    while True:
        ...
        try:
            ...
            logs.server_logger.debug(f' Принято сообщение.Клиент: {client_address} сообщение: {incoming_message}')
            #print(incoming_message)
            response = parse_message(incoming_message)
            send_message(client, response)
            client.close()
        except (ValueError, json.JSONDecodeError):
            logs.server_logger.critical(f'Ошибка! некорретное сообщение от клиента')
...
def start():
    ...
    if len(sys.argv) < 2 or sys.argv[1] != 'run':
        ...
        logs.server_logger.critical(f'Ошибка! Запуск без команды run')
        sys.exit(0)
    else:
        params = sys.argv[2:]
        print(params)
        ...
        if result[2] == 'OK':
            logs.server_logger.debug(f'приняты данные ip:{result[0]},порт:{result[1]}')
            work(result[:2])
        else:
            logs.server_logger.critical(f'Ошибка! {result[2]}')
            #print(result[2])
            sys.exit(0)
